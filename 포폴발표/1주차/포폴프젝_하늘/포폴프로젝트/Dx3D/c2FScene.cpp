...
void c2FScene::MonSterAI()
{
	...
	case Tag_Phase::Phase1:
		...
		g_pSoundManager->Play("monster footstep", DSBPLAY_LOOPING);
		CRT_Nervous = true;
	...
	case Tag_Phase::Phase2:
		...
		m_pMonster->UpdateAndRender(&m_Wolrd);
		m_pMonster->SetPosition(vPosition);
		g_pSoundManager->SetPosition("monster footstep", m_pMonster->GetPosition());
		g_pSoundManager->SetPosition("monster face", m_pMonster->GetPosition());
		g_pSoundManager->SetVolumeAndPan("monster footstep", m_vCRTPos);
		g_pSoundManager->SetVolumeAndPan("monster face", m_vCRTPos);
		break;
	case Tag_Phase::Phase3:
		CRT_Nervous = true;
		g_pSoundManager->Play("monster face", DSBPLAY_LOOPING);
	...
	case Tag_Phase::Phase4:
		CRT_Nervous = true;
		g_pSoundManager->Stop("monster footstep");
		g_pSoundManager->Stop("monster face");
